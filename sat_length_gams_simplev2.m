function [anova_sat, anova_chain, anova_combo, plot_combo, combo_text] = sat_length_gams_simplev2( mat, lipids, species, label, T, stat_yes )
...
for i=1:5
    ...
    if T>1 %% ALL EXCEPT FOR CONTROL 
        ...
        set (f6,'Position',pos2);
         ...
         f7=subplot(7,1,7); 
        bar(1:4,plot_length_(7,:)); 
        ylim([0 100]); 
        set(gca, 'XTickLabel', {'Short', 'Medium', 'Long', 'Very Long'}); 
        ylabel('Late-UnInfected'); 
        linkaxes([f6 f7],'x'); %make y axis the same
        pos1=get(f6,'Position'); %find the current position [x,y,width,height]
        pos2=get(f7,'Position'); %find the current position [x,y,width,height]
        pos2(2)=pos1(2) - pos2(4); %move the second so it touches the first 
        set (f7,'Position',pos2);
