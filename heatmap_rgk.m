function [clevels,hImage, hText, hXText] = heatmap_rgk(mat, xlab, ylab, textmat, cmax, cmin,  varargin)
...
[p, clevels] = calculateColormap(p, mat,cmin,cmax);

% Create heatmap image
p = plotHeatmap(p, mat,cmin,cmax); % New properties hImage and cdata added
...
function p = plotHeatmap(p, mat,cmin,cmax)
p.cdata = [];
if p.UseLogColormap
...
function [p, clevels] = calculateColormap(p, mat, cmin, cmax)
...
elseif ischar(p.Colormap) || isa(p.Colormap,'function_handle')
    ...
    if strcmp(p.Colormap, 'money')
        [p.Colormap, clevels] = money(mat, p.ColorLevels, cmin, cmax);
    else
...
function [cmap, clevels] = money(data, clevels, cmin, cmax)
...
elseif zeroInd >= clevels, % Just red
    b = interp1([1 clevels], [1 1], 1:clevels);
    g = interp1([1 clevels], [1 0], 1:clevels);
    r = interp1([1 clevels], [1 0], 1:clevels);
else
    b = interp1([1 zeroInd clevels], [1 1 1], 1:clevels); 
    g = interp1([1 zeroInd clevels], [1 0 0], 1:clevels);
    r = interp1([1 zeroInd clevels], [1 0 0], 1:clevels); 
end
...
function [cmap, clevels] = red(levels, cmin, cmax)
r = ones(levels, 1);
