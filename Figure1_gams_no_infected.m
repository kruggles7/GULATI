...
cd ..
cd results
mkdir ('gams_bar_charts_noinf')
cd gams_bar_charts_noinf
...
for k=1:length(unique_); 
    ...
    for j=1:3
        if j==1 %gametocytes
            ...
            plot_mat(k,3)=mean(mol_late(k,d:d+8)); %*100; 
            std_dev=std(mol_late(k,d:d+8)); 
            plot_std(k,3)=(std_dev/sqrt(9)); %*100; 
        elseif j==2 %iRBC
%             plot_mat(k,1)=mean(mol_early(k,d:d+8)); %*100; 
%             std_dev=std(mol_early(k,d:d+8)); 
%             plot_std(k,1)=(std_dev/sqrt(9)); %*100; 
...
%             plot_std(k,4)=(std_dev/sqrt(9)); %*100;   
    ...
    x_label={'Early', 'Late'}; 
    set(gca, 'XTick', [1 3]); %iRBC gray, gametocyte black
