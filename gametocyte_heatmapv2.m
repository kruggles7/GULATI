function [ plot_mat, stat_mat, stat_mat2 ] = gametocyte_heatmapv2( early, late, g_names, control ,c_names, species)
...
for j=1:r 
    name=g_names{j,1};  
    indx=find(strcmp(c_names(:,1),name)==1); 
    if numel(indx)>0
        c_mat(j,1:3)=control(indx,:); 
    end 
end 
...
for k=1:r
    ...
    plot_total(1,13:21)=mol_late(k,1:9); 
    M=median(plot_total); 
    E=zeros(9,1); 
    L=zeros(9,1);
    C=zeros(3,1); 
