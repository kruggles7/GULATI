function [hImage, hText, hXText] = heatmap_rg(mat, xlab, ylab, textmat,cmin, cmax, varargin)
...
p = calculateColormap(p, mat,cmin,cmax);
...
function p = calculateColormap(p, mat, cmin, cmax)
...
elseif ischar(p.Colormap) || isa(p.Colormap,'function_handle')
    ...
    if strcmp(p.Colormap, 'money')
        p.Colormap = money(mat, p.ColorLevels, cmin, cmax);
    else
...
function cmap = money(data, clevels, cmin, cmax)
...
function cmap = red(levels, cmin, cmax)
r = ones(levels, 1);
